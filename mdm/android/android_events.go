...
func (p *Protocol) updatePolicy(policy db.Policy) error {
	...
	devices, err := p.srv.DB.GetDevicesWithPolicy(context.Background(), policy.ID)
	if err != nil {
		panic(err)
	}
	...
	for _, udid := range devices {
		if _, err := p.ams.Enterprises.Policies.Patch(fmt.Sprintf("enterprises/%s/policies/%s", tenant.AfwEnterpriseID.String, udid), &androidPolicy).Do(); err != nil {
